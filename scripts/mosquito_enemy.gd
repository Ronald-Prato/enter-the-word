...
## Mosquito: animaciones `moving` y `attack` en `MosquitoSprite` (editar en
## `res://resources/mosquito_sprite_frames.tres`). Carga = frame 0 congelado;
## dash = resto del clip + daño por `StrikeHitArea`.
...
var _prev_ai_state: State = State.CHASE
